...
\def\esoText{The \key{Esoteric Runes} specifies the kind of action that the enchantment performs \minus{} does it create something new? Alter what is already present?  Does it give the user new abilities, or does it protect them from harm?}

\def\subText{The Subject \imp{runes} control what the {\it Action} acts upon. Does the `creation' rune form a blast of fire or a jet of water? }
...
}
{
	...
	\runeRow{iuxta}{\iuxta}{Induces a field which can detect the proximity of specified being or objects - useful for triggering effects when an item is approached or worn.}
...
{
	\runeRow{aevum}{\aevum}{Used to contain magic from the \imp{Temporal} displine}
} 
