...
\subsection{{\it Evade} or {\it Brace} }
 ...
 You may also choose to ready yourself against incoming attacks, by bolstering either of your {\it Dodge} or {\it Block} stats by the value of a Resist check. This gives you a better chance of negating incoming effects.  

See page \pageref{S:Accuracy} for more details. 
...
When an attack is launched on a being, it is necessary to determine if the target manages to successfully evade or block the attack, thereby negating it. Beings either block or dodge instinctively, without having to devote conscious thought to their reaction. These two actions are therefore termed {\it instincts}, and occur inbetween turn cycles. 

\subsection{Accuracy}

The attacker -- be they an archer, a swordsman, or a spellcaster -- quantifies their ability to successfully hit their target through an {\it accuracy check}, a standard d20 FIN (Precision) check, plus any additional relevant bonuses or penalties. 
...
\subsection{Instincts}


The accuracy check is contested by either the block or dodge statistics, which are set by:
...
Whichever you choose, you double your Expertise bonus (if applicable) and perform a Resist check, adding the value of that check into the chosen statistic for the given turn. 
...
You may only add your Expertise bonus to an accuracy check, if you are proficient in the weapon you are using, or to an {\it Instinct} if you are proficient in the armour you are wearing. 
...
Spells denote in their description if they can be blocked or dodged. For the (rarer) instances of non-spell effects which fall into one of these categories, the DM decides if it is reasonable to dodge or block the effect. 

If the `dominant' instinct (i.e. the one with the highest value) would be ineffective against a given effect, you may use the non-dominant one. However, if the character chose to, for example, use the evade action, they may not transfer the bonus to `dodge' if an evasion turns out to be ineffective. 
