...
The exception to this calculation is EVL. Since most people are (generally) much less evil than they are athletic, we use 5, rather than 10, in the denominator of the calculation. The table below gives an example:
...
As a general guide, remember that there are 10 turn cycles per minute, so a major action takes around 6 seconds to complete. Anything that takes more than 3 seconds must therefore be considered a major action. 
