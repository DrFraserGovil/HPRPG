...
~	&	\bf Brace	&	\bf Evade
\\
\cellcolor{\tablecolorhead}\bf Resist:	&	\parbox[t]{\w cm}{\raggedright Advantage on Fitness, Spirit \& Power Resist checks}	&	\parbox[t]{\w cm}{\raggedright  Advantage on Finesse, Intelligence \& Perception Resist checks}
\\
\cellcolor{\tablecolorhead}\bf Accuracy:	&	\multicolumn{2}{c}{\parbox[t]{\c cm}{\centering Agressors take disadvantage on accuracy checks made against you this turn}}
\end{rndtable}
\end{center}
\normalsize
\subsection{Cover}
...
If the `dominant' instinct (i.e. the one with the highest value) would be ineffective against a given effect, you may use the non-dominant one. However, this negates the effect of the {\it Evade} and {\it Brace} actions for the remainder of the turn cycle. 
