...
\item     Finesse (FIN): Your character{\apos}s ability to execute actions with delicacy and precision. Picking pockets, hiding and ranged attacks require finesse to execute properly. 
  ...
  \item Spirit (SPR): Your character{\apos}s ability to face down external threats without flinching, to be sure of themselves, and to resist when the odds are against them. A character with a large spirit can often resist the effects of mind-altering spells, and can summon the strength to carry on when all others would have submitted. Typically considered the defining characteristic of Gryffindor House. 
   ...
   \item Charisma (CHR): The ability of a leader, and those who influence others. Charisma helps your character convince others of what you say, and make them like and trust you. Charisma also helps cast magic that alters their perception of reality. A trait typically associated with Slytherin House.
   
   \item Intelligence (INT): Intelligence is the measure of your analytical and memory-based problem solving skills. Though not always a substitute for raw magical power, an intelligent character learns information quickly, can recall it in an instant, and can often be helpful in identifying threats. Typically considered the defining trait of Ravenclaw House.
...
{\raggedright  
	\item ATH: Acrobatics, Speed, Strength
	...
	\item PER: Compassion, Observation
...
Being considered `proficient' in an area means you are especially good at it. The mechanism of this in-game is discussed in the Actions section.
...
Health is the physical status of your character: attacking a character lowers their health, and when the health points (HP) of a character reach zero, that character is killed. A character{\apos}s initial maximum health is calculated from:
$$\text{max HP} = \text{ATH attribute} + \text{Proficiency Bonus}$$
If you have the {\it Health} proficiency, you may add three times your Expertise bonus to your base health. 

Upon levelling up, each Archetype specifies how the max HP of your character increases, this usually takes the form of a dice roll. 


{If your character is reduced to 0HP, then they acquire the Critical Condition status: they are completely immobilised, and will lose 1HP per turn. When you reach -10HP, you are dead, and nothing can bring you back. }


\subsection*{Fortitude}

Fortitude is a character{\apos}s ability to concentrate, which is necessary to cast spells and some other non-magic feats. Performing magic takes effort, and a character{\apos}s fortitude points (FP) will be slowly eroded by engaging in such mental effort.  

A character{\apos}s initial maximum mental fortitude is calculated from:
$$\text{max FP} =  \text{SPR}  \text{Proficiency Bonuses}$$
If you have the {\it Willpower} proficiency, you may add three times your Expertise bonus to your base FP. 

As with HP, each archetype specifies how much to increase your FP for each level taken in that archetype. 



Fortitude is used to cast spells, all spells have an associated fortitude cost determined by the difficulty of the spell -- as well as resist magic, and other actions which require intense concentration. You must subtract the relevant amount from your FP when performing such an action (plus or minus the appropriate amount for bonuses, power-boosted spells etc.). When your FP reaches zero, your mind is exhausted, and so you will no longer be able to engage in such complex actions. Unlike HP, FP regenerates during combat; at a rate of 2FP per combat cycle where you do not cast a spell. Outside of combat, the regeneration rate is 8FP per hour, increasing to 20 per hour whilst asleep. 



\subsection{Temporary Points}

When your HP or FP limit is raised (say, by the {\it vita maxima} spell), the current value is raised by the same amount. In contrast, when the ceiling is lowered, you only lose HP/FP if the ceiling is lowered below your current levels. 

It is never possible to have more than your maximum HP or FP unless an effect specifically states that it alters your max HP/FP.  
...
\begin{itemize}
	...
	The modifier you use to determine if an attack lands home. Typically your FIN modifier plus your Expertise bonus, though may have multiple such values for different weapons. 
	
	\item {\bf Block}
	
	One of two {\it Instincts} used to context accuracy checks. Value set to 10 + Athletics modifier.
	
	\item {\bf Dodge}
	
	The other {\it Instinct}. Value is 10 + Finesse modifier.
	
	\item {\bf Spellcasting Dice}
	
	A series of 7 values which determines the dice size you use to cast spells in each of the 7 schools of magic
	
	\item {\bf Spell Power}
	
	Determines the DV of resist checks for spells that you cast. Value is 6 + Expertise bonus + POW modifier. 
\end{itemize}
