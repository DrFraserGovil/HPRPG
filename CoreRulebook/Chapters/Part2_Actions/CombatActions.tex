...
\breaklesschapter{Combat}\index{Combat}
...
Make a \imp{Covert} roll of some kind, assuming that you are in a position where hiding is feasible (you cannot \imp{Hide} whilst out in the middle of an open field). 
...
If you are using a weapon with the \imp{Reloading} property, you must ensure that your weapon is suitably stocked with ammunition so that you may continue to fire. 

Weapons with this property have a stated `clip size' - the number of uses they may take before a \imp{Reload} action is required. When you take such an action, you refill your clip to its maximum value.
...
Any item use that can be completed in around 1 second, or which can be easily `multitasked', is considered a minor action. A common ruling would be that drinking a potion yourself takes only a \imp{Minor Action}, whilst feeding it to someone else would take a \imp{Major Action}. 
...
You do not take \imp{Drain} on any \imp{quickdefence} rolls performed this turn cycle.
