...
\breaklesschapter{Defence}\label{S:Accuracy}\label{S:Defence}
...
When you wish to defend from an attack - of any kind - you perform a DV 7 \key{Resist} check using the chosen pool, and deduct the number of successes gained from the total \imp{Power} of the attack. If the \imp{Power} of the attack reaches zero, it is nullified and the effect is cancelled. If you do not reduce the \imp{Power} to zero, then the effect is still applied, albeit at a reduced amount.  

You may defend against attacks that are directed towards you, or against someone else - as long as it is reasonable for your action to defend against another person (you cannot \key{dodge} for another being, for example - though you could use a \imp{dodge} to jump {\it into} the line of fire).
...
Generally speaking, you can only perform one \imp{Defensive} check with a single action. However, you may distribute the total number of successes up between as many attacks as you wish, as long as the \imp{GM} rules that the chosen form of defence would help negate those attacks. 

In addition, you may still use a \imp{Quick Defence} (see below) on any attack to which you have dedicated zero successes from your original defence. Doing so on the same turn as a \imp{Full Turn Defense} does not incure \imp{Drain}. 

\subsection{Early Defence}

You may attempt to defend yourself even before you spot an attack, declaring a \imp{Full Action Defence} even before 
...
Whenever you use a \imp{quickdefence} reaction without pairing it with a \imp{Full-turn defence} or a \imp{Bolstered Defence}, you add one level of \imp{Drain} to the associated statistic at the end of the turn cycle. Each level of drain reduces the \imp{rating} available, and persists until you are able to get a quick breather and take a \imp{short rest}.
...
The next time they wish to use a \imp{Quickblock}, or a \imp{Full-turn block}, their \imp{block} statistic is reduced by one level, so this character would only be able to roll 3d for a \imp{block} action. 

This process continues each time you use a \imp{quickdefence} action, reducing your effective rating each time until your level of \imp{drain} is equal to the original rating. In the example below, our character has \imp{quickdodged} three times, and so their \imp{Dodge} rating has been reduced to zero:
...
Drain is added at the end of a turn cycle, so you may perform multiple \imp{quickdefences} over the course of a single turn cycle without taking a penalty - but at the end of the round, you suffer the cumulative effects all at once. 
...
This kind of `action negation' would follow the same kind of mechanics as a defence action. The \imp{GM} should rule if it counts as a \imp{Full Round Defence} (therefore using the defence statistics and preventing \imp{drain} on your \imp{quickdefence}), or simply a normal major action, using your normal \imp{aspects}. 

It is worth emphasising explicitly (and it will be emphasised once again on page \pageref{S:CycleLimitations}) that this violates one of the key paradigms of the \imp{Combat Cycle} mechanic - namely that all actions are near-simultaneous, with all effects applied at the end of the round. In the normal course of events, therefore, it is perfectly possible for two characters to simultaneously stun each other if they cast spells on each other, or stab each other with simultaneous sword strikes.

Using the `general negation' rules presented above, however, appears to circumvent this, as your negating action manages to interrupt or alter the action being attempted. For this reason, {\bf it is impossible for a negating action to deal \imp{harm} or impose any long-lasting status effects.} Any effects manifested for the negations last only for the instant necessary to interrupt and negate the targeted action, before quickly wearing off - your punch might normally be enough to knock a rhino to the ground, but when used to interrupt and action, it is just enough to stun someone long enough to prevent them piloting a broomstick.
