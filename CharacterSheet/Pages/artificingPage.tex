...
\begin{tikzpicture}
	...
	\draw[rounded corners, \ddg, fill = \llg] ({0},{\craftTop}) rectangle ({\maxWidth},{\craftTop - \craftHeight});
	...
	The full artificing rules can be found on page \pageref{Core-S:ArtificingBasic} of the Core Rulebook.
	}};
	...
	\draw[rounded corners, \ddg, fill = \llg] ({0},{\alchemyTop}) rectangle ({\maxWidth},{\alchemyTop - \alchemyHeight});
	\node[anchor = north] at ({\maxWidth/2}, {\alchemyTop-1*\heightUnits}) {\HP \Large Potion Making};
	
	\draw[rounded corners, \ddg, fill = \lg] ({\delta},{\alchemyTop-5*\heightUnits}) rectangle ({\pouchWidth},{\alchemyTop - \alchemyHeight+\delta});
	...
	\draw[rounded corners, \ddg, fill = \mg] ({\subpouchWidth+\delta},{\alchemyTop-8.3*\heightUnits}) rectangle ({\pouchWidth-\delta},{\alchemyTop - \alchemyHeight+2*\delta});
	\node[anchor =north] at ({(\subpouchWidth+\pouchWidth)/2},{\alchemyTop-8.3*\heightUnits}) {\HP Harvested Ingredients};
	
	%\draw[rounded corners, \ddg, fill = \lg] ({\pouchWidth+\delta},{\alchemyTop-5*\heightUnits}) rectangle ({\maxWidth-\delta},{\alchemyTop - \alchemyHeight+\delta});
	...
	As described on page \pageref{Core-S:Alchemy}, all magical and alchemical ingredients have innate properties. By choosing at least three ingredients with complimentary or magically significant properties, you may mix them together to produce a potion of some kind.  
	...
	\draw[rounded corners, \ddg, fill = \llg] ({0},{\enchantTop}) rectangle ({\maxWidth},{\enchantTop - \enchantHeight});
	\node[anchor = north] at ({\maxWidth/2}, {\enchantTop-0*\heightUnits}) {\HP \Large Enchanting};
	
	\def\mod{12 * \heightUnits}
	%~ \draw[rounded corners, \ddg, fill = \mg] ({2*\runeDelta+\runeWidth},{\enchantTop-\runeDelta*2}) rectangle ({2*\runeDelta + 2*\runeWidth},{\enchantTop - \enchantHeight+\runeDelta});
	%~ \draw[rounded corners, \ddg, fill = \mg] ({3*\runeDelta+2*\runeWidth},{\enchantTop-\runeDelta*2}) rectangle ({2*\runeDelta + 3*\runeWidth},{\enchantTop - \enchantHeight+\runeDelta});
	...
	\foreach \i in {0,1,2}
	{
		...
		\draw[rounded corners, \ddg, fill = \mg] ({\xl},{\runeBoxTop}) rectangle ({\xr},{\enchantTop - \enchantHeight+\deltaMod});
