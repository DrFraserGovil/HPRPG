...
\node at (20,0.5) {\includegraphics[width = \s cm,height = \s cm]{../Images/heart}};
 \node at (24,0.5) {\includegraphics[width = \s cm,height = \s cm]{../Images/brain}};
...
\def\ry{0.5}
\def\voff{-1.3}
\draw[rounded corners, gray!30, fill = gray!10] ({\cx-\gap},{\voff+\gap}) rectangle ({\cx + \xgap + \w+\gap},{\voff-4*(\hB+\gap)});
...
\def\f{ (\width-\gapx)/3  }
\draw[rounded corners, gray!30,fill = gray!10] ({\cx-\width/2},\top) rectangle ({\cx+\width/2},{\top-\hB});
\node at (\cx,{\top-0.3}) {\Large \bf   Check Dice};
\foreach \c [count=\q from 0] in {{Charms},{Dark Arts},{Divination},{Illusion},{Malediction},{Recuperation},{Resist},{Transfiguration}}
{
	\ifthenelse{\q>3}{\def\y{ {(\top-\circleOff)}}\def\x{ {(\p+ (\q-4)*\f)} } }{\def\y{ {(\top-\circleOff+\gapy)} }\def\x{ {(\p + \q*\f)} }}
	\draw[gray!70,fill=white] (\x,\y) circle (\r);
	\node at (\x,{\y+\r+0.2}) {\small \c};
}
...
\foreach \c [count = \q from 0] in { {Arcane}, {Chaos}, {Deception}, {Dexterity}, {Endurance}, {Emotions}, {Health}, {Intimidation}, {History},{Nature}, {Performance}, {Persuasion}, {Precision},{Research}, {Speed},{Stealth},{Strength}, {Surroundings} , {Willpower} }
{
	\def\y{\topQ - \q*\gap}
	\draw (\x,\y) --(\cx,\y);
	\node[anchor = south west] (A) at (\cx,{\y-0.15}) {\scriptsize \c}; 
