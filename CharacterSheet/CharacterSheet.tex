...
\foreach \c/\a/\b [count=\q from 0] in {{Charms}/{Elemental}/{Kineses},{Divination}/{Telepathic}/{Temporal},{Illusion}/{Bewitchment}/{Psionics},{Malediction}/{Curses}/{Hexes},{Recuperation}/{Healing}/{Warding},{Transfiguration}/{Alteration}/{Conjuration},{Dark Arts}/{Necromancy}/{Occultism}}
{
	\ifthenelse{\q>3}{\def\y{ {(\top-\circleOff)}}\def\x{ {(\p+ (\q-4)*\f)} } }{\def\y{ {(\top-\circleOff+\gapy)} }\def\x{ {(\p + \q*\f)} }}
	%\draw[gray!70,fill=white] (\x,\y) circle (\r);
	\node at (\x,{\y+\r+0.2}) {\footnotesize \bf \c};
	...
	\node[anchor=west] at ({\x},{\y+\r-0.4}) {\scriptsize  \a};
	\node[anchor = west] at ({\x},{\y+\r-1}) {\scriptsize  \b};
}
\def\Qx{\p + 3*\f}
\def\Qy{\top-\circleOff}
%\node at (0,0) {\Qx \Qy};
