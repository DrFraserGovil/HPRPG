...
%%% HEROISM BOX
\draw[rounded corners,gray!70,fill=gray!10] ({\spellLimit+\widthUnits},{\heroHeight+0.5*\heightUnits})rectangle ({\secondRight},{0});
\def\yH{\heroHeight+0.4*\heightUnits}
\node[anchor=north] at ({\x},{\yH}) {\HP {Current Status}};
...
\begin{tikzpicture}
	...
	\draw[rounded corners, gray!70, fill = gray!10] ({0},{\maxHeight-1*\heightUnits}) rectangle ({\maxWidth},{\maxHeight - \alchemyHeight});
	\node[anchor = north] at ({\maxWidth/2}, {\maxHeight-1*\heightUnits}) {\HP \Large Potion Making};
	
	\draw[rounded corners, gray!70, fill = gray!20] ({\delta},{\maxHeight-5*\heightUnits}) rectangle ({\pouchWidth},{\maxHeight - \alchemyHeight+\delta});

	\node[anchor = north] at ({(\pouchWidth + \delta)/2}, {\maxHeight-5*\heightUnits}) {\HP Ingredient Pouch};
	...
	\foreach \rarity/\cost [count = \i from 1] in { {Abundant}/{~}, {Common}/{20 samples = \galleon{1}}, {Singular}/{10 samples = \galleon{1}}, {Unusual}/{3 samples = \galleon{1}}, {Rare}/{1 sample = \galleon{1}}, {Extraordinary}/{1 sample = \galleon{3}}, {Mythical}/{1 sample = \galleon{10+}} }
	{
		...
		\if \i1
			\def\ingy{\maxHeight -5*\heightUnits - \i*2*\gap}	
		...
		\else
			\def\ingy{\maxHeight -4*\heightUnits - \i*2*\gap}	
			\node[anchor = east] at ({\subpouchWidth - \delta - 11 * \boxSmall*1.42},{\ingy}) {\small \imp{\rarity{}:}}; 
	...
	\draw[rounded corners, gray!70, fill = gray!30] ({\subpouchWidth+\delta},{\maxHeight-8.3*\heightUnits}) rectangle ({\pouchWidth-\delta},{\maxHeight - \alchemyHeight+2*\delta});
	\node[anchor =north] at ({(\subpouchWidth+\pouchWidth)/2},{\maxHeight-8.3*\heightUnits}) {\HP Harvested Ingredients};
	
	\draw[rounded corners, gray!70, fill = gray!20] ({\pouchWidth+\delta},{\maxHeight-5*\heightUnits}) rectangle ({\maxWidth-\delta},{\maxHeight - \alchemyHeight+\delta});
	\node[anchor =north] at ({(\pouchWidth+\maxWidth)/2},{\maxHeight-5*\heightUnits}) {\HP Successful Recipes + Potion Notes};
\end{tikzpicture}
