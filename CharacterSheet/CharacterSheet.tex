...
\foreach \c in {1,2,3,4,5}
{
	...
	\draw[gray!70]({\leftLimit + \subBoxesDelta + \subExpGap+6*\bulletR}, {\y}) -- ({\leftLimit + \subBoxesDelta + \subBoxesWidth - \subBoxesDelta}, {\y});
}
...
{
	\def\y{\subBoxesTop - \expGap - 2.5*\boxDim - \c*\subExpGap}
	\draw[fill = black] ({\leftLimit + \subBoxesWidth + 3*\subBoxesDelta + \subExpGap}, {\y+\bulletR}) circle (\bulletR);
	\draw[gray!70]({\leftLimit + +\subBoxesWidth + 3*\subBoxesDelta + \subExpGap+6*\bulletR}, {\y}) -- ({\rightColumn - 2*\subBoxesDelta}, {\y});
}
...
%%% Status Box
\draw[rounded corners,gray!70,fill=gray!10] ({\secondColumn},{\halfUp - 0.5*\heightUnits})rectangle ({\skinnyLimit},{\halfDivider + 0.5*\heightUnits});

\def\x{(\skinnyLimit + \secondColumn)/2}
\def\yC{\halfUp - 1.5*\heightUnits}

\node at ({\x},{\yC}) {\key{Statuses \& Feats}};
