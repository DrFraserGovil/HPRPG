...
\def\defaultSetter
{
	\setkeys{beast}{name = none, species = None, mind = none, rating = Unset, category = none, description = None, fit =0, prs = 0, vit =0, cha = 0, dec = 0, ins = 0, int = 0, wil =0, pcp = 0, block = 0, dodge = 0, nUnharmed =0, nBruised = 0, nHurt = 0, nInjured = 0, nWounded = 0, nMangled = 0,fortitude = 0}
}
\newcounter{yCount}
...
{%q
    ...
    \begin{tikzpicture}
       ...
       \foreach [count = \j from 0] \a/\b in { {Unharmed}/\nUnharmed, {Bruised \\(-1)}/\nBruised, {Hurt\\ (-2)}/\nHurt, {Injured(-3)}/\nInjured, {Wounded \\(-4)}/\nWounded, {Mangled (-5)}/\nMangled, {Critical}/1}
       {
			\def\xVal{ {(\theyCount + (\b)/2)*\bx} + \thenonZeroCount*\bx/4}
			\def\lx { {\theyCount *\bx} + \thenonZeroCount*\bx/4}
			\if \b0
				%%
			\else
...
\newcommand\stat[2]
{
	\parbox[t]{\w cm}{\vspace{0.3cm} \centering \key{#1} \\ \ratingB{#2}}
}
\newcommand\statBlock
{
